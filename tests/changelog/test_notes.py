...
def test_duplicate_target_is_ambiguous_even_when_identical(consumer, location):
    ...
    if location == "same":
        ...
        archive.write_text(text + DEFINITION, newline="\n")
    path.write_text(text + DEFINITION, newline="\n")
...
def test_check_cli_is_read_only_and_checks_archives(consumer, capsys):
    ...
    archive.write_text(archive.read_text().replace("[1.0.0]", "[0.9.0]"), newline="\n")
